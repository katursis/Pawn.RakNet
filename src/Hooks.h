...
private:
    ...
    static bool THISCALL HOOK_RakServer__Send(void *_this, RakNet::BitStream *bitStream,
                                              int priority, int reliability,
                                              char orderingChannel, PlayerID playerId,
                                              bool broadcast);
    static bool THISCALL HOOK_RakServer__RPC(void *_this, RPCIndex *uniqueID,
                                             RakNet::BitStream *bitStream,
                                             int priority, int reliability,
                                             char orderingChannel, PlayerID playerId,
                                             bool broadcast, bool shiftTimestamp);
    static Packet * THISCALL HOOK_RakServer__Receive(void *_this);
    static void * THISCALL HOOK_RakServer__RegisterAsRemoteProcedureCall(void *_this,
                                                                         RPCIndex *uniqueID,
